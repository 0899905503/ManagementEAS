...
class WorkingTime {
  ...
  String get formattedTime {
    ...
    if (hours < 1) return "$min ${S.current.minutes}";
    return "$hours ${S.current.hours} ${min - (hours * 60)} ${S.current.minutes}";
  }
  ...
  String get lackTimeFormat {
    ...
    int hours = (min / 60).floor();
    if (hours < 1) {
      return "${isLackTime ? "-" : "+"} $min ${S.current.minutes}";
    }
    return "${isLackTime ? "-" : "+"} $hours ${S.current.hours} ${min - (hours * 60)} ${S.current.minutes}";
  }
